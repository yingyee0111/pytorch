...
""" Triton Implementation of the flex_attention_variant Kernel"""
...
def flex_attention_variant_grid(batch_size, q_heads, num_queries, d_model, meta):
...
def build_subgraph_buffer(
    ...
    raise ValueError("FlexAttentionVariant was passed a subgraph with no output node!")


# Inner Triton functions shared by flex_attention_variant & split-k decoding kernels.
...
compute_flex_attention_variant = r"""
...
flex_attention_variant_template = TritonTemplate(
    name="flex_attention_variant",
    grid=flex_attention_variant_grid,
    source=compute_flex_attention_variant
...
def _use_flex_decoding(query, kernel_options):
    ...
    return (
        not kernel_options.get("FORCE_USE_flex_attention_variant", False)
...
@register_lowering(torch.ops.higher_order.flex_attention_variant, type_promotion_kind=None)
def flex_attention_variant(
    ...
    k_subgraph_buffer = build_subgraph_buffer(
        k_placeholder_inps + list(k_mod_other_buffers), k_subgraph
    )
    q_placeholder_inps = [
        create_placeholder(name, dtype, query.get_device())
        for name, dtype in [
            ("query", query.get_dtype()),
            ("b", torch.int32),
            ("h", torch.int32),
            ("m", torch.int32),
            ("n", torch.int32),
    ...
    q_subgraph_buffer = build_subgraph_buffer(
        q_placeholder_inps + list(q_mod_other_buffers), q_subgraph
    )
    ...
    k_mod_other_buffers = maybe_realize(k_mod_other_buffers)
    ...
    # because they're implicitly added by the q_mod/ k_mod function
    ...
    for BLOCK_M, BLOCK_N, num_warps, num_stages in configs:
        ...
        flex_attention_variant_template.maybe_append_choice(
            choices=choices,
            input_nodes=[
                query,
                key,
...
def flex_attention_variant_backward_grid(
...
flex_attention_variant_backward_template = TritonTemplate(
    name="flex_attention_variant_backward",
    grid=flex_attention_variant_backward_grid,
    source=r"""
...
# TODO: We probably also need a layout constraint?
@register_lowering(
    torch.ops.higher_order.flex_attention_variant_backward, type_promotion_kind=None
)
def flex_attention_variant_backward(*args, **kwargs):
    (
        query,
        key,
        value,
    ...
    k_fw_subgraph_buffer = build_subgraph_buffer(
        k_fwd_placeholder_inps + list(k_mod_other_buffers), k_fw_graph
    )

    k_joint_placeholder_inps = k_fwd_placeholder_inps + [
        create_placeholder("grad_k_mod", dtype, device)
    ]
    k_joint_subgraph_buffer, *_ = build_subgraph_buffer(
        k_joint_placeholder_inps + list(k_mod_other_buffers), k_joint_graph
    )
    ...
    q_fw_subgraph_buffer = build_subgraph_buffer(
        q_fwd_placeholder_inps + list(q_mod_other_buffers), q_fw_graph
    )

    q_joint_placeholder_inps = q_fwd_placeholder_inps + [
        create_placeholder("grad_q_mod", dtype, device)
    ]
    q_joint_subgraph_buffer, *_ = build_subgraph_buffer(
        q_joint_placeholder_inps + list(q_mod_other_buffers), q_joint_graph
    )
    ...
    for BLOCK1, BLOCK2, num_warps, num_stages in configs:
        ...
        flex_attention_variant_backward_template.maybe_append_choice(
            choices=choices,
            input_nodes=[
                query,
                key,
            ...
            subgraphs=[k_fw_subgraph_buffer, k_joint_subgraph_buffer, q_fw_subgraph_buffer, q_joint_subgraph_buffer, mask_graph_buffer],
