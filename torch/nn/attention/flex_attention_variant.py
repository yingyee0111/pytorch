...
# flake8: noqa C101
"""This module implements the user facing API for flex_attention_variant in PyTorch."""
import functools
import inspect
import itertools
import math
...
from torch._higher_order_ops.flex_attention_variant import flex_attention_variant as flex_attention_variant_hop
...
_qk_mod_signature = Callable[[Tensor, Tensor, Tensor, Tensor, Tensor], Tensor]
...
class _ModificationType(Enum):
    ...
    - QK_MOD: q_mod or k_mod function which accepts a score as the first argument
...
def _get_mod_type(fn: Callable) -> _ModificationType:
    ...
    considered as a q_mod/ k_mod function. If the function has 4 positional arguments, it is
    ...
    if num_positional_args == 5:
        return _ModificationType.QK_MOD
...
def _vmap_for_bhqkv(
    ...
    """Used to vmap q_mods/ k_mods and mask_mods over 4-dimensional/5-dimension inputs.
...
def create_mask(
    mod_fn: Union[_qk_mod_signature, _mask_mod_signature],
    B: Optional[int],
    H: Optional[int],
    Q_LEN: int,
    KV_LEN: int,
    ...
    Args:
        mod_fn (Union[_qk_mod_signature, _mask_mod_signature]): Function to modify attention scores.
        B (int): Batch size.
    ...
    with TransformGetItemToIndex():
        if mod_type == _ModificationType.QK_MOD:
            qk_mod = mod_fn
            qk_mod = _vmap_for_bhqkv(qk_mod, prefix=(0,))  # first input is score
            out = qk_mod(torch.zeros(B, H, Q_LEN, KV_LEN, device=device), b, h, m, n)
...
def create_block_mask(
    ...
    Example Usage:
        .. code-block:: python
            ...
            output = flex_attention_variant(query, key, value, block_mask=block_mask)
    """
...
def _validate_device(query: Tensor, key: Tensor, value: Tensor):
    ...
    if query.device.type != "cuda":
        raise ValueError(
            "FlexAttentionVariant is only supported on CUDA devices. "
            f"Found input tensors on {query.device.type} device."
        )


def flex_attention_variant(
    query: Tensor,
    key: Tensor,
    value: Tensor,
    k_mod: Optional[_qk_mod_signature] = None,
    q_mod: Optional[_qk_mod_signature] = None,
    ...
    r"""This function implements scaled dot product attention with an arbitrary attention q/k modification function.

    This function computes the scaled dot product attention between query, key, and value tensors with a user-defined
    attention q/k modification function. The attention q/k are calculated as follows:

    The ``q_mod``/ ``k_mod`` function should have the following signature:
    ...
    Where:
        - ``q``/ ``k``: A scalar tensor representing the query / key,
          with the same data type and device as the query, key, and value tensors.
        - ``batch``, ``head``, ``q_idx``, ``k_idx``, ``emb_idx``: Scalar tensors indicating
          the batch index, query head index, query index, key index, and embedding index respectively.
    ...
    Args:
        ...
        k_mod (Optional[Callable]): Function to modify attention keys. By default no k_mod is applied.
        q_mod (Optional[Callable]): Function to modify attention queries. By default no q_mod is applied.
    ...
    .. warning::
        `torch.nn.attention.flex_attention_variant` is a prototype feature in PyTorch.
    ...
    if torch.compiler.is_dynamo_compiling():
        ...
        out, lse = flex_attention_variant_hop(
            query, key, value, k_mod, q_mod, block_mask.as_tuple(), scale, kernel_options  # type: ignore[union-attr]
        )
        if return_lse:
    ...
    if not torch._dynamo.is_dynamo_supported():
        raise RuntimeError("flex_attention_variant requires dynamo support")
    ...
    def _flex_attention_variant_hop_wrapper(*args, **kwargs):
        return flex_attention_variant_hop(*args, **kwargs)
            ...
            with _temp_remove_pre_dispatch_torch_function_mode():
                with _temp_remove_metadata_torch_function_mode() as metadata_mode:
                    ...
                    out, lse = torch.compile(
                        _flex_attention_variant_hop_wrapper, backend=backend, fullgraph=True
                    )(
                        query,
                        key,
                        value,
